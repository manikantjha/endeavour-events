export const companyName = "Endavour Events";

export const lstNavbarMenu = [
  { id: 1, name: "Home", path: "/" },
  { id: 2, name: "About", path: "/about" },
  { id: 3, name: "Services", path: "/services" },
  { id: 4, name: "Work", path: "/work" },
  { id: 5, name: "Testimonials", path: "/testimonials" },
  { id: 6, name: "Contact", path: "/contact" },
];

export const lstFigures = [
  { id: 1, figure: "100+", description: "Events Produced" },
  { id: 2, figure: "6000+", description: "Largest Audience" },
  { id: 3, figure: "63+", description: "Most Speakers" },
  { id: 4, figure: "90M+", description: "Money Raised" },
];

export const lstTestimonials = [
  {
    id: 1,
    client: {
      imgSrc:
        "https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=761&q=80",
      name: "John Doe",
      designation: "Owner at XYZ",
    },
    title: "Lorem ipsum dolor sit amet.",
    testimonial:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Provident, nihil.",
  },
  {
    id: 2,
    client: {
      imgSrc:
        "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80",
      name: "John Doe",
      designation: "Owner at XYZ",
    },
    title: "Lorem ipsum dolor sit amet.",
    testimonial:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Provident, nihil.",
  },
  {
    id: 3,
    client: {
      imgSrc:
        "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80",
      name: "John Doe",
      designation: "Owner at XYZ",
    },
    title: "Lorem ipsum dolor sit amet.",
    testimonial:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Provident, nihil.",
  },
  {
    id: 4,
    client: {
      imgSrc:
        "https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=761&q=80",
      name: "John Doe",
      designation: "Owner at XYZ",
    },
    title: "Lorem ipsum dolor sit amet.",
    testimonial:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Provident, nihil.",
  },
];

export const lstFeatures = [
  {
    id: 1,
    icon: (
      <svg
        className="w-10 h-10 mb-2 text-primary"
        aria-hidden="true"
        fill="currentColor"
        viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fillRule="evenodd"
          d="M5 5a3 3 0 015-2.236A3 3 0 0114.83 6H16a2 2 0 110 4h-5V9a1 1 0 10-2 0v1H4a2 2 0 110-4h1.17C5.06 5.687 5 5.35 5 5zm4 1V5a1 1 0 10-1 1h1zm3 0a1 1 0 10-1-1v1h1z"
          clipRule="evenodd"
        ></path>
        <path d="M9 11H3v5a2 2 0 002 2h4v-7zM11 18h4a2 2 0 002-2v-5h-6v7z"></path>
      </svg>
    ),
    title: "Feature A",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex, exercitationem.",
  },
  {
    id: 2,
    icon: (
      <svg
        className="w-10 h-10 mb-2 text-primary"
        aria-hidden="true"
        fill="currentColor"
        viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fillRule="evenodd"
          d="M5 5a3 3 0 015-2.236A3 3 0 0114.83 6H16a2 2 0 110 4h-5V9a1 1 0 10-2 0v1H4a2 2 0 110-4h1.17C5.06 5.687 5 5.35 5 5zm4 1V5a1 1 0 10-1 1h1zm3 0a1 1 0 10-1-1v1h1z"
          clipRule="evenodd"
        ></path>
        <path d="M9 11H3v5a2 2 0 002 2h4v-7zM11 18h4a2 2 0 002-2v-5h-6v7z"></path>
      </svg>
    ),
    title: "Feature B",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex, exercitationem.",
  },
  {
    id: 3,
    icon: (
      <svg
        className="w-10 h-10 mb-2 text-primary"
        aria-hidden="true"
        fill="currentColor"
        viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fillRule="evenodd"
          d="M5 5a3 3 0 015-2.236A3 3 0 0114.83 6H16a2 2 0 110 4h-5V9a1 1 0 10-2 0v1H4a2 2 0 110-4h1.17C5.06 5.687 5 5.35 5 5zm4 1V5a1 1 0 10-1 1h1zm3 0a1 1 0 10-1-1v1h1z"
          clipRule="evenodd"
        ></path>
        <path d="M9 11H3v5a2 2 0 002 2h4v-7zM11 18h4a2 2 0 002-2v-5h-6v7z"></path>
      </svg>
    ),
    title: "Feature C",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex, exercitationem.",
  },
  {
    id: 4,
    icon: (
      <svg
        className="w-10 h-10 mb-2 text-primary"
        aria-hidden="true"
        fill="currentColor"
        viewBox="0 0 20 20"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          fillRule="evenodd"
          d="M5 5a3 3 0 015-2.236A3 3 0 0114.83 6H16a2 2 0 110 4h-5V9a1 1 0 10-2 0v1H4a2 2 0 110-4h1.17C5.06 5.687 5 5.35 5 5zm4 1V5a1 1 0 10-1 1h1zm3 0a1 1 0 10-1-1v1h1z"
          clipRule="evenodd"
        ></path>
        <path d="M9 11H3v5a2 2 0 002 2h4v-7zM11 18h4a2 2 0 002-2v-5h-6v7z"></path>
      </svg>
    ),
    title: "Feature D",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex, exercitationem.",
  },
];

export const lstServices = [
  {
    id: 1,
    title: "Service A",
    checklist: [
      "Lorem, ipsum dolor.",
      "Lorem ipsum dolor sit amet.",
      "Lorem ipsum dolor sit amet consectetur adipisicing.",
      "Lorem ipsum dolor sit amet consectetur.",
    ],
  },
  {
    id: 2,
    title: "Service B",
    checklist: [
      "Lorem, ipsum dolor.",
      "Lorem ipsum dolor sit amet.",
      "Lorem ipsum dolor sit amet consectetur adipisicing.",
      "Lorem ipsum dolor sit amet consectetur.",
    ],
  },
  {
    id: 3,
    title: "Service C",
    checklist: [
      "Lorem, ipsum dolor.",
      "Lorem ipsum dolor sit amet.",
      "Lorem ipsum dolor sit amet consectetur adipisicing.",
      "Lorem ipsum dolor sit amet consectetur.",
    ],
  },
];
